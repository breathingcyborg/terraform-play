...
resource "aws_ecs_service" "service" {
  name            = "tf_play_7_ecs_service"
  task_definition = aws_ecs_task_definition.task_def.arn
  cluster         = aws_ecs_cluster.cluster.id
  desired_count   = 1

  # Dont update desired_count back to 1
  lifecycle {
  ...
  load_balancer {
    ...
    container_name   = "tf_play_7_hello_world"
    container_port   = 3000
  }
}

resource "aws_cloudwatch_log_group" "ecs_log_group" {
  name = "/ecs/tf_play_7_hello_world"
}
...
resource "aws_ecs_task_definition" "task_def" {
  family = "tf_play_7_service" # unique name for this task def
...
resource "aws_ecs_capacity_provider" "capacity_provider" {
  name = "tf_play_7_capacity_provider"
...
resource "aws_ecs_cluster_capacity_providers" "cluster_capacity_provider_association" {
  cluster_name       = "tf_play_7_cluster"
  capacity_providers = ["tf_play_7_capacity_provider"]
