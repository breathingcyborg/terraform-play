...
resource "aws_ecs_service" "service" {
  ...
  load_balancer {
    ...
    container_name   = "tf_play_5_hello_world"
    container_port   = 3000
  }
}

resource "aws_cloudwatch_log_group" "ecs_log_group" {
  name = "/ecs/tf_play_5_hello_world"
}
...
locals {
  ecr_repo = aws_ecr_repository.repo
  ecr_repo_dualstack_url =  "${local.ecr_repo.registry_id}.dkr-ecr.${local.ecr_repo.region}.on.aws/${local.ecr_repo.name}:latest"
}

resource "aws_ecs_task_definition" "task_def" {
  family = "tf_play_service" # unique name for this task def
  ...
  container_definitions = jsonencode([
    {
      name      = "tf_play_5_hello_world"
      image     = "${local.ecr_repo_dualstack_url}"
      essential = true
      memory    = 300 # MB
...
resource "aws_ecs_capacity_provider" "capacity_provider" {
  name = "tf_play_5_capacity_provider"
...
resource "aws_ecs_cluster_capacity_providers" "cluster_capacity_provider_association" {
  cluster_name       = "tf_play_5_cluster"
  capacity_providers = ["tf_play_5_capacity_provider"]
